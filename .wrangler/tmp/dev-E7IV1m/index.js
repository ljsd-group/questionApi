...
// .wrangler/tmp/bundle-H3Vo60/checked-fetch.js
...
// .wrangler/tmp/bundle-H3Vo60/strip-cf-connecting-ip-header.js
...
var PostgresJsPreparedQuery = class extends PgPreparedQuery {
  ...
  constructor(client2, queryString, params, logger, fields, customResultMapper) {
...
var PostgresJsTransaction = class _PostgresJsTransaction extends PgTransaction {
  ...
  transaction(transaction) {
    return this.session.client.savepoint((client2) => {
      const session = new PostgresJsSession(client2, this.dialect, this.schema, this.session.options);
...
function Socket() {
  ...
  function write(data, cb) {
    tcp.writer.write(data).then(cb, error2);
    return true;
  }
  __name(write, "write");
  function end(data) {
...
function Connection(options, queues = {}, { onopen = noop, onend = noop, onclose = noop } = {}) {
  ...
  function ErrorResponse(x) {
    query && (query.cursorFn || query.describeFirst) && write(Sync);
    const error3 = Errors.postgres(parseError(x));
    query && query.retried ? errored(query.retried) : query && query.prepared && retryRoutines.has(error3.routine) ? retry(query, error3) : errored(error3);
  }
...
// src/config/index.js
var getDatabaseUrl = /* @__PURE__ */ __name(() => {
...
var success = /* @__PURE__ */ __name((data = null, message = "success") => {
  return {
    code: 200,
    message,
    data
...
var error = /* @__PURE__ */ __name((message = "error", status = 500) => {
  return {
    code: status,
    message,
    data: null
...
var validateDeviceId = /* @__PURE__ */ __name((deviceId) => {
...
check.post("/check-device", async (c) => {
  try {
    ...
    const existingResponse = await db.select().from(responses).where(eq(responses.deviceId, deviceId)).limit(1);
...
var submit = new Hono2();
var getCurrentTime = /* @__PURE__ */ __name(() => {
  const now = /* @__PURE__ */ new Date();
  const timezoneOffset = now.getTimezoneOffset();
  now.setHours(now.getHours() + 8);
  const utcTime = new Date(now.getTime() + timezoneOffset * 60 * 1e3);
  const utc8Time = new Date(utcTime.getTime() + 8 * 60 * 60 * 1e3);
  return utc8Time;
}, "getCurrentTime");
var convertTimestampToUTC8 = /* @__PURE__ */ __name((timestamp2) => {
  try {
    ...
    return new Date(date.getTime() + 8 * 60 * 60 * 1e3);
  } catch (err) {
    console.error("\u65F6\u95F4\u6233\u8F6C\u6362\u5931\u8D25:", err, "\u65F6\u95F4\u6233:", timestamp2);
    return getCurrentTime();
  }
}, "convertTimestampToUTC8");
submit.post("/submit-response", async (c) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      const existingResponse = await tx.select().from(responses).where(eq(responses.deviceId, deviceId)).limit(1);
      let response;
      if (existingResponse.length > 0) {
        [response] = await tx.update(responses).set({
          language,
          completedAt: getCurrentTime()
        }).where(eq(responses.deviceId, deviceId)).returning();
        await tx.delete(answers).where(eq(answers.responseId, response.id));
      } else {
        [response] = await tx.insert(responses).values({
          deviceId,
          language,
          completedAt: getCurrentTime()
        }).returning();
...
app.route("/api", check_default);
app.route("/api", submit_default);
app.onError((err, c) => {
...
// .wrangler/tmp/bundle-H3Vo60/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-H3Vo60/middleware-loader.entry.ts
